...
public class CommandHandler {
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
    ...
    // First ensure at least the command and prefix is present,
    // the arg length can be handled by your command function
    ...
    if (argArray[0].startsWith(BotUtils.DEFAULT_PREFIX)) {
      ...
      // Extract the "command" part of the first arg out by ditching the amount of characters
      ...
      // Instead of delegating the work to a switch, automatically do it via calling the mapping if
      // it exists
      ...
      if (event.getChannel().isPrivate() && event.getAuthor().getLongID() == BotUtils.OWNER_ID) {
        ...
        // Instead of delegating the work to a switch, automatically do it via calling the mapping
        // if it exists
