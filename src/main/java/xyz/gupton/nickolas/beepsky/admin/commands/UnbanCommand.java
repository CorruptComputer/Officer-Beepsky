...
public class UnbanCommand implements Command {
  ...
  public void execute(Guild guild, User author, MessageChannel channel, String message) {
    ...
    if (!BotUtils.isBanned(userId.asString())) {
      BotUtils.sendOwnerMessage("Error Unbanning", Globals.CLIENT.getUserById(userId).block().getUsername()
          + " is not banned.");
      return;
    }
    ...
    try {
      ...
      BotUtils.sendOwnerMessage("Unban Successful", Globals.CLIENT.getUserById(userId).block().getUsername()
              + " has been unbanned.");
    } catch (Exception e) {
      e.printStackTrace();
      BotUtils.sendOwnerMessage("Error Unbanning", e.getMessage());
    }
  }
